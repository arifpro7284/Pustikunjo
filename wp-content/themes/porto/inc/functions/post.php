...
if ( ! function_exists( 'porto_pagination' ) ) :
	function porto_pagination( $max_num_pages = null, $load_more = false, $query = false, $current_link = false, $current_page = false, $is_editor = false ) {
		...
		/**
		 * Work with an unescaped url here.
		 * `esc_url()` encodes `&` as the `&#038;` html entity and `add_query_arg()`, which
		 * `remove_query_arg()` and `paginate_links()` rely on, treats the `#` of that entity as the
		 * start of a url fragment. That moves the rest of the query string to the end of the link,
		 * e.g. `?s=c&post_type=product` turns into `?s=c&post_type=product#038;post_type=product/page/2/`.
		 * `paginate_links()` escapes the links it returns, so the base must stay raw.
		 */
		if ( $current_link ) {
			$page_num_link = esc_url_raw( html_entity_decode( $current_link ) );
		} else {
			$page_num_link = html_entity_decode( get_pagenum_link( 1, false ) );
		}
		$query_args = array();
		...
		$page_num_link = esc_url_raw( remove_query_arg( array_keys( $query_args ), $page_num_link ) );

		// Any remaining query string has to stay behind the `%_%` placeholder, otherwise the page
		// number would be appended after it, e.g. `?foo=bar/page/2/`.
		$link_parts    = explode( '?', $page_num_link, 2 );
		$page_num_link = trailingslashit( $link_parts[0] ) . '%_%' . ( isset( $link_parts[1] ) && '' !== $link_parts[1] ? '?' . $link_parts[1] : '' );
	...
	function porto_ajax_posts() {
		...
		if ( $is_shortcode ) {
			...
			if ( $posts->have_posts() ) {
				...
				porto_pagination( null, false, $posts, ! empty( $_REQUEST['current_link'] ) ? esc_url_raw( wp_unslash( $_REQUEST['current_link'] ) ) : false, ! empty( $_REQUEST['page'] ) ? (int) $_REQUEST['page'] : false );

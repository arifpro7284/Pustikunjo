...
/**
 * Porto AI Setup-Wizard module. Loaded here (not only from setup_wizard.php,
 * which is behind an is_admin()/admin_init gate) so the porto/v1/ai/* REST
 * routes register on REST requests too. admin.php itself loads for any
 * manage_options user, including REST, so this is REST-reachable.
 */
require_once PORTO_ADMIN . '/setup_wizard/ai/bootstrap.php';
...
class Porto_Admin {
	...
	public function check_purchase_code() {
		...
		if ( ! $this->_checkedPurchaseCode ) {
			$code         = isset( $_POST['code'] ) ? porto_strip_tags( wp_unslash( $_POST['code'] ) ) : '';
			$code_confirm = $this->get_purchase_code();

			if ( isset( $_POST['action'] ) && ! empty( $_POST['action'] ) ) {
				if ( ! $code || $code !== $code_confirm ) {
					if ( $code_confirm && 'unregister' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ) {
				...
				if ( $code ) {
					// One field, one path: the Porto API validates both Envato purchase
					// codes and PortoTheme.com (Freemius) license keys.
					...
					if ( ! $result ) {
						...
						$this->_checkedPurchaseCode = isset( $result['message'] ) ? $result['message'] : __( 'License Key or Purchase Code is not valid.', 'porto' );
						$code_confirm               = '';
					}
			...
			} elseif ( $code && $code_confirm && $code === $code_confirm ) {
				$this->_checkedPurchaseCode = 'verified';
			}
		}
		return $this->_checkedPurchaseCode;
	...
	public function curl_purchase_code( $code, $act ) {
		...
		if ( 'add' === $act || 'remove' === $act ) {
			$args['site_url'] = base64_encode( home_url() );
			$args['version']  = PORTO_VERSION;
		}

		$url    = add_query_arg( $args, $this->activation_url );
		$result = $importer_api->get_response( $url );
	...
	/**
	 * @param string $code Purchase or license code.
	 * @return bool
	 */
	public function is_envato_purchase_code_format( $code ) {
	...
	public function clear_license_data() {
		$this->clear_envato_purchase_code();
		delete_option( 'porto_license_source' );
	}

	public function clear_envato_purchase_code() {
		delete_option( 'envato_purchase_code_9207399' );
	}

	/**
	 * Verify license before Porto API downloads (validated by the Porto API server).
	 *
	 * @return true|WP_Error
	 */
	public function verify_license_for_download() {
		if ( ! $this->is_registered() ) {
			return new WP_Error(
				'not_registerd',
				__( 'Please <a href="admin.php?page=porto">register</a> Porto theme to get access to pre-built demo websites and auto updates.', 'porto' )
			);
		}
	...
	function get_purchase_code_asterisk() {
		...
		if ( $this->is_envato_purchase_code_format( $code ) ) {
			return substr( $code, 0, 13 ) . '-****-****-************';
		}
		// License key: reveal only the first and last 4 characters.
		$len = strlen( $code );
		if ( $len <= 8 ) {
	...
	public function upgrader_pre_download( $reply, $package, $obj ) {
		...
		if ( strpos( $package, $importer_api->get_url( 'theme' ) ) !== false || strpos( $package, $importer_api->get_url( 'plugins' ) ) !== false ) {
			$verify = $this->verify_license_for_download();

...
/**
 * WPBakery 9.0 turned the border width and the element width into range params, the border width
 * value can carry its px unit and the .vc_sep_width_* classes were dropped from the css in favour
 * of an inline width, so the element width is rendered as a style below.
 */
$border_width = str_replace( 'px', '', (string) $border_width );
...
$inline_style = ( '' !== $el_width && 100 != $el_width ) ? ' style="width: ' . esc_attr( (float) $el_width ) . '%;"' : '';
?>
<div class="<?php echo esc_attr( trim( $css_class ) ); ?>"<?php echo porto_filter_output( $inline_style ); ?>>

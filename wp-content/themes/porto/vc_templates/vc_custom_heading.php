...
if ( $enable_link_dynamic ) {
	if ( ( 'meta_field' == $link_dynamic_source ) && ! empty( $link_dynamic_content_meta_link ) ) {
		$link = trim( Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $link_dynamic_source, $link_dynamic_content_meta_link, 'link' ) );
	}
	if ( ! empty( $link_dynamic_content ) ) {
		$link = trim( Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $link_dynamic_source, $link_dynamic_content, 'link' ) );
	}
	if ( empty( $link ) ) {
...
if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) || $show_border ) {
	...
	$output .= force_balance_tags( wp_kses_post( wp_specialchars_decode( $text ) ) );
	$output .= '</' . $tag . '>';

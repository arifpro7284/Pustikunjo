...
$output = '';
/**
 * WPBakery 9.0 replaced the `options` list by the `striped` / `animated` params, dropped the
 * preset `bgcolor` and added the text shadow params. They are initialized for the older versions.
 */
$options = $bgcolor = $striped = $animated = $add_text_shadow = $text_shadow_color = '';
...
if ( 'true' === $animated || in_array( 'animated', $options, true ) ) {
	$bar_options[] = 'animated';
}
if ( 'true' === $striped || in_array( 'striped', $options, true ) ) {
	$bar_options[] = 'striped';
}
...
// since 9.0 there is no preset bgcolor anymore, the picked colors are always used.
if ( '' !== $custombgcolor && ( 'custom' === $bgcolor || '' === $bgcolor ) ) {
	$custombgcolor = vc_get_css_color( 'background-color', $custombgcolor );
	if ( '' !== $customtxtcolor ) {
		$style_text_color  = vc_get_css_color( 'color', $customtxtcolor );
		$style_text_shadow = ( 'true' === $add_text_shadow && $text_shadow_color ) ? 'text-shadow: 0 -1px 0 ' . esc_attr( $text_shadow_color ) . ';' : '';
...
$element_class    = empty( $this->getSettings()['element_default_class'] ) ? '' : $this->getSettings()['element_default_class'];
$class_to_filter  = 'vc_progress_bar ' . esc_attr( $element_class );
$class_to_filter .= vc_shortcode_custom_css_class( $css, ' ' ) . $this->getExtraClass( $el_class ) . ' ' . $this->getCSSAnimation( $css_animation );
$css_class        = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, trim( $class_to_filter ), $this->getSettings()['base'], $atts );
...
foreach ( $values as $data ) {
	...
	if ( isset( $data['customtxtcolor'] ) && ( ! isset( $data['color'] ) || 'custom' === $data['color'] ) ) {
		// the per value text shadow was added in WPBakery 9.0.
		$style_text_color     = 'color: ' . esc_attr( $data['customtxtcolor'] ) . ';';
		$style_text_shadow    = ( ! empty( $data['add_text_shadow'] ) && 'true' === $data['add_text_shadow'] && ! empty( $data['text_shadow_color'] ) ) ? 'text-shadow: 0 -1px 0 ' . esc_attr( $data['text_shadow_color'] ) . ';' : '';
...
foreach ( $graph_lines_data as $line ) {
	...
	if ( $unit ) {
		if ( $tooltip ) {
			wp_enqueue_script( 'porto-bar-tip' );

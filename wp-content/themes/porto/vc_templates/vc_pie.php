 ...
 */
$title            = $output = '';
// the preset color param was dropped in WPBakery 9.0, only the colorpicker is left.
$color            = $custom_color = '';
...
if ( 'default' == $type ) {
	...
	if ( $custom_color && ( '' === $color || 'custom' === $color ) ) {
		// 9.0 has no preset colors anymore, the picked color is used as is.
		$color = $custom_color;
	} else {
	...
	if ( $is_wpb_rendering ) {
		...
		$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class . ' ' . vc_shortcode_custom_css_class( $css, ' ' ), $this->getSettings()['base'], $atts );
	}

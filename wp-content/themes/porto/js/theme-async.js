...
( function( theme, $ ) {
	...
	Carousel.prototype = {
		...
		build: function() {
			...
			// Responsive Item Margin: override owl's per-breakpoint margin when set ( options.margin_md /
			// margin_sm / margin_xs come from the Porto Carousel block's responsive Item Margin control ).
			var porto_bp_margin = { 768: this.options.margin_md, 576: this.options.margin_sm, 0: this.options.margin_xs };
			for ( var porto_bp in porto_bp_margin ) {
				if ( typeof porto_bp_margin[porto_bp] != 'undefined' && porto_bp_margin[porto_bp] !== '' && porto_bp_margin[porto_bp] !== null && responsive[porto_bp] ) {
					responsive[porto_bp].margin = Number( porto_bp_margin[porto_bp] );
				}
			}
	...
	$.extend( theme, {
		...
		PostFilter: {
			...
			load_posts: function( cat, is_infinite, $parent, post_type, $posts_wrap, default_args, page_url ) {
				...
				var pid = $parent.attr( 'id' ) || $parent.data( 'pid' ),
					self = this,
					is_archive = $parent.hasClass( 'archive-posts' );
				if ( ! pid ) {
					if ( typeof theme.PostFilter.load_posts.pid == 'undefined' ) {
						theme.PostFilter.load_posts.pid = 0;
					}
					theme.PostFilter.load_posts.pid += 1;
					pid = 'porto_posts_grid_cache_' + theme.PostFilter.load_posts.pid;
					$parent.data('pid', pid);
				}
				var successfn = function( res, directcall ) {
						if ( !res ) {
							return;
						}

	...
	( function( theme, $ ) {
		...
		$( function() {
			...
			$( document ).on( 'added_to_cart', 'body', function( event ) {
				...
				// open minicart on single product page
				if ( this.classList.contains( 'single-product' ) ) {
					var $minicart_obj = $( '#mini-cart.open-minicart-added:visible' );
					if ( $minicart_obj.length && $( '.sp-only-cart-form' ).length ) {
						if ( $minicart_obj.closest( '.header-main.sticky' ).length && $( '#header' ).hasClass( 'scroll-down' ) ) {
							$( window ).scrollTop( $( window).scrollTop() - 1 );
						}
						setTimeout(() => {
							$( '#header' ).removeClass( 'scroll-down' );
							$minicart_obj.find( '.cart-head' ).trigger( 'click' );
						}, 150);
					}
				}
			} );

			// Fix stuck offcanvas mini cart after pressing Back from cart/checkout.
			// On mobile bfcache restore the single-product page comes back with the
			// mini cart still "opened" (overlay + html overflow:hidden). With a
			// "Scroll Up" sticky header the restored `scroll-down` class hides the
			// header — and the mini cart panel inside it — while the fixed overlay
			// stays, leaving the page stuck behind an un-closable overlay. The
			// added_to_cart event never re-fires on restore, so reset the mini cart
			// and sticky-header scroll state here to make the page usable again.
			$( window ).on( 'pageshow.portoMinicartReset', function( e ) {
				if ( ! ( e.originalEvent && e.originalEvent.persisted ) ) {
					return; // Only on bfcache restore; a fresh load is already clean.
				}
				var $offcanvas = $( '.minicart-offcanvas.minicart-opened' );
				if ( $offcanvas.length ) {
					$offcanvas.removeClass( 'minicart-opened' );
					$( 'html' ).css( { 'overflow': '', 'margin-right': '' } );
				}
				$( '#mini-cart.open' ).removeClass( 'open' );
				$( '#header' ).removeClass( 'scroll-down' );
				$( '.page-wrapper' ).removeData( 'prev-scrollpos' );
			} );
				...
				events: function() {
					...
					if ( typeof wc_add_to_cart_params != 'undefined' ) {
						$( document.body ).on( 'wc_cart_button_updated', function( e, $button ) {
							if ( $button.closest( '.sp-only-cart-form' ).length ) {
								$button.closest( '.sp-only-cart-form' ).find( '.added_to_cart' ).addClass( 'wc-action-btn view-cart-btn button wc-forward' );
							}
						});
						$( document.body ).on( 'click', '.sp-only-cart-form .single_add_to_cart_button:not(.disabled, .wpcbn-btn), .single-product .single_add_to_cart_button:not(.disabled, .wpcbn-btn)', function( e ) {

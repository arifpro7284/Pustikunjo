...
jQuery( function( $ ) {
						...
						if ( 'g' == importdata.type ) {
							...
							try {
								...
								if ( e.message.indexOf( 'porto_hb_myaccount' ) > -1 || e.message.indexOf( 'porto_products_filter' ) > -1 ||  e.message.indexOf( 'porto_product_attribute_filter' ) > -1 || e.message.indexOf( 'porto_hb_mini_cart' ) > -1 ) {
									...
									window.alert( e.message + wp.i18n.__( '. In Full Site Editing(Builder Mode), we provide posts grid widget instead.', 'porto' ) );
								} else {
	...
	if ( document.body.classList.contains( 'block-editor-page' ) ) {
		// Apply the page's custom CSS ( #custom_css ) inside the editor. The modern block editor
		// renders content in an iframe ( name="editor-canvas" ), so appending the <style> only to the
		// main document head has no effect on the block preview; inject it into the iframe document
		// head as well ( and keep the main-head copy for the non-iframed editor ).
		var portoGetCustomCSS = function() {
			return $( '#custom_css' ).length ? String( $( '#custom_css' ).val() || '' ).replace( /<script[\s\S]*?<\/script>/gi, '' ) : '';
		};
		var portoApplyCustomCSSToDoc = function( doc ) {
			if ( !doc || !doc.head ) {
				return;
			}
			var css = portoGetCustomCSS(),
				styleEl = doc.getElementById( 'porto_custom_css' );
			if ( !css ) {
			...
			if ( !styleEl ) {
				...
				styleEl.id = 'porto_custom_css';
				doc.head.appendChild( styleEl );
			}
			styleEl.textContent = css;
		};
		var portoEditorCanvasDoc = function() {
			var iframe = document.querySelector( 'iframe[name="editor-canvas"]' );
			return iframe && iframe.contentDocument ? iframe.contentDocument : null;
		};
		var portoApplyCustomCSS = function() {
			portoApplyCustomCSSToDoc( document );
			portoApplyCustomCSSToDoc( portoEditorCanvasDoc() );
		};
		...
		if ( wp && wp.data && typeof wp.data.subscribe == 'function' ) {
			...
			wp.data.subscribe( function() {
				// The editor-canvas iframe mounts asynchronously and re-mounts on device / preview
				// changes, dropping the injected <style>; re-apply the custom CSS when it is missing.
				var canvasDoc = portoEditorCanvasDoc();
				if ( canvasDoc && portoGetCustomCSS() && ! canvasDoc.getElementById( 'porto_custom_css' ) ) {
					portoApplyCustomCSSToDoc( canvasDoc );
				}
	...
	if ( typeof porto_builder_condition != 'undefined' && ( location.pathname.indexOf( 'post.php' ) > -1 || location.pathname.indexOf( 'post-new.php' ) > -1 ) ) {
		...
		if ( typeof elementor != 'undefined' ) {
			...
			// Open Porto Studio only when the post genuinely has no content.
			// The previous approach checked the painted DOM after a fixed 150ms,
			// but on the Post Type Builder the blocks are usually not rendered yet
			// at that point, so an existing post looked "empty" and Studio opened
			// on every load. Instead, wait for the editor store to load the post
			// and read the actual content.
			if ( typeof wp != 'undefined' && wp.data && typeof wp.data.subscribe == 'function' && wp.data.select( 'core/editor' ) ) {
				var portoStudioFirstChecked = false;
				var portoStudioUnsub = wp.data.subscribe( function() {
					if ( portoStudioFirstChecked ) {
						return;
					}
					var editor = wp.data.select( 'core/editor' );
					var post = ( editor && typeof editor.getCurrentPost == 'function' ) ? editor.getCurrentPost() : null;
					// Wait until the post entity is loaded into the store.
					if ( ! post || typeof post.id == 'undefined' ) {
						return;
					}
					portoStudioFirstChecked = true;
					if ( typeof portoStudioUnsub == 'function' ) {
						portoStudioUnsub();
					}
					// The saved content is hydrated together with the post object, so
					// it is reliable even before the blocks are painted. Edited
					// content is only an extra guard (it can never cause a false open).
					var savedContent = '';
					if ( post.content ) {
						savedContent = ( typeof post.content == 'string' ) ? post.content : ( post.content.raw || '' );
					}
					var editedContent = ( typeof editor.getEditedPostContent == 'function' ) ? editor.getEditedPostContent() : '';
					if ( '' !== savedContent.trim() || '' !== editedContent.trim() ) {
						return; // content exists — do not auto-open Studio
					}
					// Open Studio once its toolbar button has been appended.
					var portoStudioAttempts = 0;
					( function portoOpenStudio() {
						var $trigger = $( '#gutenberg-porto-studio-trigger' );
						if ( $trigger.length ) {
							$trigger.trigger( 'click' );
						} else if ( portoStudioAttempts++ < 50 ) {
							setTimeout( portoOpenStudio, 100 );
						}
					} )();
				} );
			}

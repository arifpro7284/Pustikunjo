...
add_filter( 'pre_http_request', function( $pre, $args, $url ){
	if ( strpos( $url, 'https://sw-themes.com/activation/porto_wp/download/' ) !== false ) {
		...
		if ( 'plugins_version.php' === $basename ) {
			...
			$url = "https://dl.gpltimes.com/file/gpltimes/porto/{$get_args['demo']}.zip";
			return wp_remote_get( $url, $args );
		} elseif ( 'block_categories.php' === $basename ) {
			$url = "https://dl.gpltimes.com/file/gpltimes/porto/porto-studio/block_categories_{$get_args['type']}.json";
			return wp_remote_get( $url, $args );
		} elseif ( 'blocks.php' === $basename ) {
			$url = "https://dl.gpltimes.com/file/gpltimes/porto/porto-studio/blocks_{$get_args['type']}.json";
			return wp_remote_get( $url, $args );
		} elseif ( 'block_content.php' === $basename ) {
			$url = "https://dl.gpltimes.com/file/gpltimes/porto/porto-studio/{$get_args['block_id']}.json";
			return wp_remote_get( $url, $args );
		}
	}
	return $pre;
...
if ( is_admin() ) {
	...
	add_action( 'enqueue_block_assets', 'porto_admin_block_css', 1000 );
	...
	function porto_admin_block_css() {
		if ( wp_style_is( 'porto_admin' ) || wp_style_is( 'porto_admin', 'registered' ) ) {
...
function porto_css() {
	...
	wp_register_style( 'porto-animate', PORTO_URI . '/css/part/animate.css', array( 'porto-theme' ), PORTO_VERSION );
...
function porto_scripts() {
	...
	if ( ! is_admin() && ! in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ) ) ) {
		...
		if ( function_exists( 'vc_is_inline' ) && vc_is_inline() ) {
			$vc_editor_deps = array( 'porto-theme' );
			if ( function_exists( 'vc_is_page_editable' ) && vc_is_page_editable() && wp_script_is( 'vc_inline_iframe_js', 'registered' ) ) {
				/**
				 * Inside the frontend editor iframe `page_editable.js` assigns `window.vc_iframe`,
				 * which would wipe the `vc_iframe` api polyfills of this script.
				 */
				$vc_editor_deps[] = 'vc_inline_iframe_js';
			}
			wp_enqueue_script( 'porto-vc-frontend-editor', PORTO_JS . '/admin/vc-frontend-editor.js', $vc_editor_deps, PORTO_VERSION, true );
		}

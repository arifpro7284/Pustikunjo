/**
 * AI content generator — sends prompts to the WordPress server-side proxy
 * (action: porto_ai_generate), which calls OpenAI Chat Completions. The API
...
jQuery( function( $ ) {
	...
	function portoAiEscHtml( s ) {
		return String( s ).replace( /&/g, '&amp;' ).replace( /</g, '&lt;' ).replace( />/g, '&gt;' );
	}

	/**
	 * Insert generated text into the editor.
	 * - excerpt           -> the post excerpt field
	 * - content / outline -> the post content (Gutenberg blocks or Classic editor)
	 *
	 * @return bool Whether a target editor was found.
	 */
	function portoAiInsertContent( type, text ) {
		if ( ! text ) {
			return false;
		}
		...
		if ( 'excerpt' == type ) {
			if ( window.wp && wp.data && wp.data.select( 'core/editor' ) ) {
				wp.data.dispatch( 'core/editor' ).editPost( { excerpt: text } );
				return true;
			}
			var $excerpt = $( '#excerpt' );
			if ( $excerpt.length ) {
				$excerpt.val( text ).trigger( 'change' );
				return true;
			}
			return false;
		}

		// Gutenberg block editor -> append paragraph blocks.
		if ( window.wp && wp.data && wp.blocks && wp.data.select( 'core/block-editor' ) ) {
			var blocks = paragraphs.map( function ( p ) {
				return wp.blocks.createBlock( 'core/paragraph', { content: portoAiEscHtml( p ).replace( /\n/g, '<br>' ) } );
			} );
		...
		if ( window.tinymce && tinymce.get( 'content' ) && ! tinymce.get( 'content' ).isHidden() ) {
			var html = paragraphs.map( function ( p ) {
				return '<p>' + portoAiEscHtml( p ).replace( /\n/g, '<br>' ) + '</p>';
			} ).join( '' );
			tinymce.get( 'content' ).execCommand( 'mceInsertContent', false, html );
			return true;
		}

		// Classic editor (text mode textarea).
		var $content = $( '#content' );
		if ( $content.length ) {
	...
	$( 'body' ).on( 'click', '.porto-ai-dialog:not(.loading) .btn-insert', function (e) {
		e.preventDefault();
		var $btn = $( this ),
			$dialog = $( '.porto-ai-dialog' ),
			text = $.trim( $( '#ai-output' ).val() ),
			type = $dialog.attr( 'data-ai-type' );
		if ( ! text ) {
			return;
		}
		if ( portoAiInsertContent( type, text ) ) {
			$btn.html( wp.i18n.__( 'Inserted', 'porto-functionality' ) );
			setTimeout( function () {
				$dialog.addClass( 'hide' );
			}, 600 );
		} else {
			window.alert( wp.i18n.__( 'Could not find the editor to insert into. Please copy the text manually.', 'porto-functionality' ) );
		}
	} );
	...
	$( 'body' ).on( 'click', '.ai_generate, .button-plugin-gen', function ( e ) {
		...
		if ( typeof js_porto_admin_vars != 'undefined' && js_porto_admin_vars.ai_enabled && js_porto_admin_vars.ai_nonce ) {
			var __ = wp.i18n.__,
				aiSettings = {
			...
			// Only content/outline/excerpt have a clear editor target — SEO meta
			// fields (and SEO-plugin generations) don't, so hide Insert for those.
			var insertable = ( 'ai_generate' == generateType ) && ( -1 !== $.inArray( contentType, [ 'content', 'outline', 'excerpt' ] ) );
			$dialog.attr( 'data-ai-type', contentType ).find( '.btn-insert' ).css( 'display', insertable ? '' : 'none' );

			// Edit dialog title
			$dialog.removeClass( 'hide' ).addClass('loading');
			$outText.val( '' );
			$dialog.find( '.btn-insert' ).html( __( 'Insert into Editor', 'porto-functionality' ) );
			...
			// The assembled prompt is sent to the WordPress server-side proxy
			// (action: porto_ai_generate); the OpenAI key stays on the server.
			var data = {
				action: 'porto_ai_generate',
				nonce: js_porto_admin_vars.ai_nonce,
			...
			$.ajax( {
				url: js_porto_admin_vars.ajaxurl || window.ajaxurl,
				type: 'POST',
				dataType: 'json',
				timeout: 35000,
				data: data
				...
				if ( response && response.success && response.data && response.data.text ) {
					$outText.val( response.data.text.trim() );
				} else {
					var errorMessage = ( response && response.data && response.data.message )
						? response.data.message
						: __( 'Generate Failed! There is a problem with your prompt.', 'porto-functionality' );

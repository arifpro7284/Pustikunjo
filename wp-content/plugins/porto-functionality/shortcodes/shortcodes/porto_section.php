	...
	register_block_type(
		...
		array(
			...
			'render_callback' => function( $atts, $content = null ) {
				...
				if ( ! empty( $settings['remove_padding'] ) ) {
					$classes[] = 'porto-section-no-padding';
				}
				...
				if ( ! $settings['add_container'] ) {
					...
					if ( ! empty( $atts['gap'] ) ) {
						$classes[] = 'porto-style-gap-' . PortoShortcodesClass::get_global_hashcode( $atts['gap'], '', array(), 8 );
					}
				}
				...
				$tag = isset( $settings['tag'] ) ? strtolower( sanitize_key( $settings['tag'] ) ) : 'div';
				if ( ! in_array( $tag, array( 'section', 'div', 'article' ), true ) ) {
					$tag = 'div';
				}

				// Style Options > Animation ( appear-animation data attributes ).
				$attrs .= apply_filters( 'porto_add_animation_attrs', '', 'porto_section', $atts );

				$output = '<' . $tag . ' class="' . esc_attr( apply_filters( 'porto_elements_wrap_css_class', implode( ' ', $classes ), $atts, 'section' ) ) . '"' . $attrs . '>';

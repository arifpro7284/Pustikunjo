<?php
	...
	if ( $settings['title'] || $icon_cls ) {
		...
		if ( ( $settings['nofollow'] ) || ( ! empty( $settings['link'] ) && ! empty( $settings['link']['nofollow'] ) ) ) {
			$attrs .= ' rel="nofollow"';
		}
		...
		$link_title = '';
		if ( ! empty( $settings['link'] ) && ! empty( $settings['link']['custom_attributes'] ) && class_exists( '\Elementor\Utils' ) ) {
			$attributes = array();
			// Custom URL attributes should come as a string of comma-delimited key|value pairs.
			$attributes = array_merge( $attributes, \Elementor\Utils::parse_custom_attributes( $settings['link']['custom_attributes'] ) );
			if ( ! empty( $attributes ) ) {
				foreach( $attributes as $key => $value ) {
					$link_title .= ' ' . $key . '="' . esc_attr( $value ) . '"';
				}
			}
		}
		if ( ! empty( $settings['aria_text'] ) ) {
			$link_title .= ' aria-label="' . $settings['aria_text'] . '"';
		}
		echo '<' . $tag . ' class="' . esc_attr( apply_filters( 'porto_elements_wrap_css_class', $btn_classes, $atts, 'button' ) ) . '" href="' . esc_url( $url ) . '"' . ( isset( $settings['link']['is_external'] ) && $settings['link']['is_external'] ? ' target="_blank"' : '' ) . porto_shortcode_add_floating_options( $atts ) . $attrs . $link_title . '>';

...
if ( ! function_exists( 'porto_ultimate_heading_spacer' ) ) {
	...
	if ( $enable_field_dynamic ) {
		...
		if ( ! empty( $main_heading ) && is_array( $main_heading ) ) {
			$main_heading = $field_dynamic_before . apply_filters( 'porto_dynamic_el_render_array_value', implode( '<span class="sep">, </span>', $main_heading ), $main_heading ) . $field_dynamic_after;
		} else {
	...
	if ( ! empty( $enable_marquee ) ) {
		...
		$type_plugin .= ' class="marquee' . ( ! empty( $mq_pause_hover ) ? ' mq-pause-hover' : '' ) . '" data-duration="' . floatval( $marquee_duration ) . '" data-gap="10" data-duplicated="true" data-startVisible="true" data-clone-element="5"';
	}
	...
	if ( 'line_only' == $spacer ) {
		// Line Width is either a legacy string/number ( "auto" / "200" ) or the new
		// PortoAdvancedRangeControl responsive object
		// ( array( 'desktop' => array( 'value', 'unit' ), 'tablet' => ..., 'mobile' => ... ) ).
		// Normalize both shapes to CSS width strings ( unit included ) so old saved
		// values keep rendering after the control switch.
		$lw_resp    = array();
		$lw_allowed = array( 'px', '%', 'em', 'rem', 'vw', 'vh' );
		if ( is_array( $line_width ) ) {
			foreach ( array( 'desktop', 'tablet', 'mobile' ) as $lw_dev ) {
				if ( ! empty( $line_width[ $lw_dev ] ) && isset( $line_width[ $lw_dev ]['value'] ) && '' !== (string) $line_width[ $lw_dev ]['value'] ) {
					$lw_unit            = ( isset( $line_width[ $lw_dev ]['unit'] ) && in_array( $line_width[ $lw_dev ]['unit'], $lw_allowed, true ) ) ? $line_width[ $lw_dev ]['unit'] : 'px';
					$lw_resp[ $lw_dev ] = floatval( $line_width[ $lw_dev ]['value'] ) . $lw_unit;
				}
			}
			$wrap_width = isset( $lw_resp['desktop'] ) ? $lw_resp['desktop'] : '';
		} elseif ( '' !== (string) $line_width ) {
			$wrap_width = ( 'auto' === $line_width ) ? 'auto' : floatval( $line_width ) . 'px';
		} else {
			$wrap_width = '';
		}
		...
		if ( ! empty( $wrap_width ) ) {
			$line_style_inline .= 'width:' . esc_attr( $wrap_width ) . ';';

			// Responsive Line Width ( tablet / mobile ) for the new advanced-range object.
			if ( ! empty( $lw_resp['tablet'] ) ) {
				$custom_style .= ' @media(max-width:' . $breakpoints['md'] . '){ .' . esc_attr( trim( $custom_class ) ) . ' .porto-u-headings-line{ width:' . esc_attr( $lw_resp['tablet'] ) . '; } }';
			}
			if ( ! empty( $lw_resp['mobile'] ) ) {
				$custom_style .= ' @media(max-width:' . $breakpoints['sm'] . '){ .' . esc_attr( trim( $custom_class ) ) . ' .porto-u-headings-line{ width:' . esc_attr( $lw_resp['mobile'] ) . '; } }';
			}
	...
	if ( ( is_array( $floating_img ) && ! empty( $floating_img['id'] ) ) ) {
		...
		if ( empty( $image_info ) || empty( $image_info['src'] ) ) { // For Skip Media
			$image_info['src'] = PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg';
		}
		...
		if ( empty( $image_info ) || empty( $image_info['src'] ) ) {
			$image_info['src'] = PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg';
		}
	...
	if ( $main_heading ) {
		...
		if ( 'yes' == $enable_highlight ) {
			if ( '' == $enable_svghighlight ) {
				...
				$main_heading_hlight = ' data-appear-animation="highlightShape"';
				$svg_shape = '';
				...
				} elseif ( 'underline2' == $highlight_svg ) {
					$svg_shape = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 504.01 35.73" preserveAspectRatio="none"><path d="M2.01,33.72c0,0,129.21-3.27,234.55-25.26s69.99,19.3,69.99,19.3s120.08-21.63,195.46-0.12"/></svg>';
				}
				$main_heading = '<span class="heading-highlight-before">' . wp_kses_post( $before_heading ) . '</span><span class="heading-highlight-svg">' . wp_kses_post( $main_heading ) . '<span class="svg-highlight ' . esc_attr( $highlight_svg ) . '">' . $svg_shape . '</span></span><span class="heading-highlight-after">' . wp_kses_post( $after_heading ) . '</span>';
			}
		}

...
if ( 'schedule' == $type ) {
...
} elseif ( 'timeline' == $type ) {
	global $porto_timeline_count;
	$porto_timeline_count = 0;
	$el_class             = porto_shortcode_extract_class( $el_class );
	$result                       .= '<section class="step-timeline timeline' . esc_attr( $el_class ) . ( empty( $shortcode_class ) ? '' : ' ' . trim( esc_attr( $shortcode_class ) ) ) . '">';
		$result                   .= '<div class="timeline-body">';
		ob_start();
		include porto_shortcode_template( 'porto_step_item' );
		$result                   .= ob_get_clean();
	...
	unset( $GLOBALS['porto_timeline_count'] );
}

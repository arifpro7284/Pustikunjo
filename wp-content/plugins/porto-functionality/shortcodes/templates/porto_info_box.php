...
if ( $link ) {
	if ( is_array( $link ) && isset( $link['url'] ) ) {
		...
		if ( ! empty( $link['custom_attributes'] ) && class_exists( '\Elementor\Utils' ) ) {
			...
			$attributes = array_merge( $attributes, \Elementor\Utils::parse_custom_attributes( $link['custom_attributes'] ) );
			if ( ! empty( $attributes ) ) {
...
if ( $link ) {
	if ( 'box' == $read_more ) {
		$html = '<a' . ( ! empty( $title ) ? ' aria-label="' . esc_html( $title ) . '"' : '' ) . ' class="porto-sicon-box-link' . ( ! empty( $hidden_cls ) ? $hidden_cls : '' ) . '" href="' . esc_url( $url ) . '"' . $target . $rel . $link_title . '>' . $html . '</a>';
	}
}

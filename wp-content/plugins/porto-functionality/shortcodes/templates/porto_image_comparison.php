...
if ( ! empty( $atts['before_img'] ) ) {
	if ( is_numeric( $atts['before_img'] ) ) {
		$image_html = wp_get_attachment_image( $atts['before_img'], 'full', false, array( 'class' => 'porto-image-comparison-before' ) );
		if ( empty( $image_html ) ) {
			$image_html = '<img class="porto-image-comparison-before" src="' . PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg' . '">';
		}
		echo porto_filter_output( $image_html );
	} else {
		echo '<img class="porto-image-comparison-before" src="' . $atts['before_img'] . '">';
	}
}

if ( ! empty( $atts['after_img'] ) ) {
	if ( is_numeric( $atts['after_img'] ) ) {
		$image_html = wp_get_attachment_image( $atts['after_img'], 'full', false, array( 'class' => empty( $atts['before_img'] ) ? 'porto-image-comparison-before' : 'porto-image-comparison-after' ) );
		if ( empty( $image_html ) ) {
			$image_html = '<img class="porto-image-comparison-after" src="' . PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg' . '">';
		}
		echo porto_filter_output( $image_html );
	} else {
		echo '<img class="porto-image-comparison-after" src="' . $atts['after_img'] . '">';
	}
}

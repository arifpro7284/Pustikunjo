...
// In the block editor pipeline the inner blocks are pre-rendered BEFORE this container's
// callback runs, so the preset globals ( $porto_grid_layout / $porto_item_count ) set above
// were empty for them. Re-render the inner blocks now that those globals are set so each
// grid item picks up its creative-grid classes. WPBakery renders container-first, so it
// keeps using do_shortcode( $content ).

...
if ( ! function_exists( 'vc_iconpicker_type_porto' ) ) {
	...
	function vc_iconpicker_type_porto( $icons ) {
		$porto_icons = array(
			...
			array( 'porto-icon-bird' => 'Bird' ),
			array( 'porto-icon-cat' => 'Cat' ),
			array( 'porto-icon-dog' => 'Dog' ),
			array( 'porto-icon-fish' => 'Fish' ),
			array( 'porto-icon-pet' => 'Pet' ),
			array( 'porto-icon-special-offer' => 'Special Offer' ),
			array( 'porto-icon-candle' => 'Candle' ),
			array( 'porto-icon-cup' => 'Cup' ),
			array( 'porto-icon-pamela-hat' => 'Pamela Hat' ),
			array( 'porto-icon-potted-plant' => 'Potted Plant' ),
			array( 'porto-icon-price-tag' => 'Price Tag' ),
			array( 'porto-icon-textile' => 'Text Tile' ),
			array( 'porto-icon-wood-plank' => 'Wood Plank' ),
...
if ( ! function_exists( 'porto_vc_iconpicker_type_fontawesome5' ) ) {
	function porto_vc_iconpicker_type_fontawesome5( $icons ) {
		global $porto_settings_optimize;
		if ( ! empty( $porto_settings_optimize['optimize_fontawesome'] ) ) {
			$fontawesome_icons = array(
				...
				'Regular' => array(
					...
					array( 'far fa-comment-alt' => 'Comment Alt(speech, notification, note, chat, bubble, feedback, message, texting, sms, conversation)' ),
...
function porto_number_callback( $settings, $value ) {
	...
	if ( ! empty( $units ) ) {
		...
		<input type="number"
			...
			value="<?php echo ( isset( $responsive_value['xl'] ) ? esc_attr( $responsive_value['xl'] ) : '' ); ?>"
	...
	if ( $is_responsive ) {
		...
		<div class="porto-responsive-dropdown">
			...
			<ul class="porto-responsive-span">
				<li data-width="xl" class="active" data-size="100%"><i class="vc-composer-icon vc-c-icon-layout_default"></i></li>
				<li data-width="lg" title="< <?php echo porto_get_xl_width(); ?>px" data-size="1024px"><i class="vc-composer-icon vc-c-icon-layout_landscape-tablets"></i></li>
				<li data-width="md" title="< 992px" data-size="768px"><i class="vc-composer-icon vc-c-icon-layout_portrait-tablets"></i></li>
				<li data-width="sm" title="< 768px" data-size="480px"><i class="vc-composer-icon vc-c-icon-layout_landscape-smartphones"></i></li>
			...
			foreach ( $custom_fonts as $c_fonts ) {
				if ( ! empty( $c_fonts ) ) {
					foreach ( $c_fonts as $c_font_name => $font_fields ) {
						$_font = str_replace( '+', ' ', $c_font_name );
						if ( ! in_array( $_font, $fonts ) ) {
							$fonts[] = $_font;
						}
...
/**
 * Whether an element can hold other elements.
 *
 * Mirrors the container test WPBakery uses in its editor scripts:
 ...
 * @return bool
 */
function porto_wpb_element_is_container( $shortcode ) {
	if ( ! class_exists( 'WPBMap' ) || ! WPBMap::exists( $shortcode ) ) {
		return false;
	}

	$settings = WPBMap::getShortCode( $shortcode );

	if ( ! is_array( $settings ) ) {
		return false;
	}

	return ( isset( $settings['is_container'] ) && true === $settings['is_container'] ) || ! empty( $settings['as_parent'] );
}

/**
 * Whether an element gets the paste control in the backend editor.
 *
 * Pasting puts the copied element *inside* the target - `vc.pasteShortcode()` passes the target id on
 * as the `parent_id` of the pasted element - so only containers can offer it. That is also why
 * WPBakery gives it to vc_row, vc_section, vc_column and the tta elements and to nothing else.
 *
 * @since 7.6.0
 *
 * @param string $shortcode Shortcode tag.
 * @return bool
 */
function porto_wpb_element_has_paste_control( $shortcode ) {
	$has_control = 0 === strpos( $shortcode, 'porto_' ) && porto_wpb_element_is_container( $shortcode );

	/**
	 * Filters which elements get the paste control.
	 *
	 * Only pass elements that can contain other elements, a paste into a non container element
	 * creates a child of an element that never renders its children.
	 *
	 * @since 7.6.0
	 *
	 * @param bool   $has_control Whether the element gets the control.
	 * @param string $shortcode   Shortcode tag.
	 */
	return (bool) apply_filters( 'porto_wpb_element_has_paste_control', $has_control, $shortcode );
}

/**
 * Add the paste control to an element rendered from its `controls_list`.
 *
 * Note that the Porto containers do NOT come through here. Only elements built on WPBakeryShortCode
 * render their controls from `controls_list`; WPBakeryShortCodesContainer, which every Porto
 * container extends, renders them in `getColumnControls()` from `get_column_control_settings()`,
 * which has no filter at all. Those get the control from
 * assets/js/porto-wpb-container-paste.js instead.
 *
 * This is kept so that `porto_wpb_element_has_paste_control` also works for anyone who opts a non
 * container element in, and it is what WPBakery itself uses for the tta elements.
 *
 * `copypaste` is not used here on purpose. Its dropdown is only turned into a select2 by
 * `initCopyPasteSelect()`, which the backend editor calls from the row, section and tta views only,
 * so on any other element it would render as a bare select. The plain `paste` control needs nothing
 * extra: editors/partials/backend_controls.tpl.php already renders it and the base element view
 * `vc.shortcode_view` already binds `click .column_paste, .vc_control-btn-paste` to its generic
 * `vc.pasteShortcode()` handler.
 *
 * @since 7.6.0
 *
 * @param array  $controls  Control list.
 * @param string $shortcode Shortcode tag.
 * @return array
 */
function porto_wpb_element_controls_list( $controls, $shortcode ) {
	if ( ! is_array( $controls ) || ! is_string( $shortcode ) ) {
		return $controls;
	}
	if ( in_array( 'paste', $controls, true ) || in_array( 'copypaste', $controls, true ) ) {
		return $controls;
	}
	if ( ! porto_wpb_element_has_paste_control( $shortcode ) ) {
		return $controls;
	}

	$controls = array_values( $controls );
	$index    = array_search( 'copy', $controls, true );

	if ( false === $index ) {
		// No copy control either, add both of them in front of delete.
		$index = array_search( 'delete', $controls, true );
		array_splice( $controls, false === $index ? count( $controls ) : $index, 0, array( 'copy', 'paste' ) );
	} else {
		array_splice( $controls, $index + 1, 0, array( 'paste' ) );
	}

	return $controls;
}
add_filter( 'vc_wpbakery_shortcode_get_controls_list', 'porto_wpb_element_controls_list', 10, 2 );

/**
 * Take over the `column_offset` param of the column elements.
 *
 * WPBakery 9.0 preselects the legacy `width` attribute on the `md` row (>= 992px) of the
 * Responsiveness tab although it is rendered as a `vc_col-sm-*` class (>= 768px).
 * See lib/wpb-controls/column-offset.php.
 *
 * WPBakery registers the param on the same hook with the default priority.
 *
 * @since 7.6.0
 */
function porto_wpb_load_column_offset_param() {
	if ( ! class_exists( 'Vc_Column_Offset' ) || ! function_exists( 'vc_add_shortcode_param' ) ) {
		return;
	}

	require_once PORTO_SHORTCODES_LIB . 'wpb-controls/column-offset.php';

	if ( function_exists( 'porto_wpb_column_offset_form_field' ) ) {
		vc_add_shortcode_param( 'column_offset', 'porto_wpb_column_offset_form_field' );
	}
}
add_action( 'vc_load_default_params', 'porto_wpb_load_column_offset_param', 20 );

/**
 * Get the WPBakery elements Porto keeps as regular elements although WPBakery marks them deprecated.
 *
 * @since 7.6.0
 *
 * @return array Shortcode tags.
 */
function porto_wpb_restored_elements() {
	/**
	 * Filters the deprecated WPBakery elements Porto restores as regular elements.
	 *
	 * Add 'vc_tabs' and 'vc_tour' here to restore the old tabs / tour elements as well.
	 *
	 * @since 7.6.0
	 *
	 * @param array $tags Shortcode tags.
	 */
	return apply_filters( 'porto_wpb_restored_elements', array( 'vc_accordion' ) );
}

/**
 * Restore the deprecated 'Old Accordion' element as a regular element.
 *
 * WPBakery 9.0 added `'content_element' => false` to config/deprecated/shortcode-vc-accordion.php,
 * which keeps [vc_accordion] mapped - so existing content still renders and can be edited - but drops
 * it from the "Add Element" panel. Porto still ships the styles and scripts for it, so put it back.
 *
 * The `deprecated` setting is removed as well. Besides the "You are using outdated element" notice in
 * the edit form, it makes Vc_Add_Element_Box add the `vc_element-deprecated` class to the button, and
 * the name search of the panel filters those out with `:not(".vc_element-deprecated")` - so the
 * element is browsable by category but never found by keyword. That one can't be solved in css, the
 * button is excluded from the result set instead of being hidden.
 *
 * Runs on the settings of a single element, right after WPBakery loaded its (lean mapped) config file
 * and before the element is marked as initialized.
 *
 * @since 7.6.0
 *
 ...
 * @return array
 */
function porto_wpb_restore_element_settings( $settings, $tag ) {
	if ( is_array( $settings ) && in_array( $tag, porto_wpb_restored_elements(), true ) ) {
		$settings['content_element'] = true;
		unset( $settings['deprecated'] );
	}
	return $settings;
}
add_filter( 'vc_element_settings_filter', 'porto_wpb_restore_element_settings', 20, 2 );

...
function porto_theme_vc_animation_type_field( $settings, $value ) {
	...
	$options     = array( 'hinge', 'rollIn', 'rollOut', 'textEntrance' );
	foreach ( $options as $option ) {
		$selected = '';
		if ( $option == $value ) {
			$selected = ' selected="selected"';
...
/**
 * Generate Escaped Responsive style
 * 
 * @since 3.9.0
 */
if ( ! function_exists( 'porto_build_style_res_declaration' ) ) {
	function porto_build_style_res_declaration( $device_val, $property ) {
		if ( empty( $device_val ) || empty( $device_val['value'] ) ) {
			return '';
		}
		$unit = 'px';
		if ( isset( $device_val['unit'] ) ) {
			$unit = $device_val['unit'];
		}
		// $unit can be "%", which sanitize_key() strips ( it only keeps [a-z0-9_-] ). Sanitize the
		// unit ourselves, allowing letters and "%" so valid CSS units ( %, px, em, rem, vw, vh, … )
		// survive while still blocking any injection.
		$unit = preg_replace( '/[^a-zA-Z%]/', '', (string) $unit );
		return sanitize_key( $property ) . ':' . floatval( $device_val['value'] ) . $unit . ';';
	}
}

...
foreach ( $filter_areas as $index => $area ) {
	...
	if ( 'category' == $area ) {
		if ( 'list' == $display_type ) {
			...
			echo '<label class="sr-only" for="product_cat">' . esc_html__( 'Product Category Dropdown', 'porto-functionality' ) . '</label>';
		...
		if ( 'list' == $display_type ) {
			...
			echo '<select aria-label="Price Range" class="porto_dropdown_price_range" name="min_price">';
		...
		if ( 'list' == $display_type ) {
			...
			echo '<select class="porto_dropdown_product_attributes" aria-label="' . esc_attr( $area ) . '" name="filter_' . esc_attr( $area ) . '">';

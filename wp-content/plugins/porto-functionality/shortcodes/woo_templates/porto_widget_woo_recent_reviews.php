...
if ( ! wp_style_is( 'porto-woo-widget-recent-review-products' ) && defined( 'PORTO_URI' ) ) {
	porto_enqueue_link_style( 'porto-woo-widget-recent-review-products', PORTO_URI . '/css/part/woo-widget-recent-review-products.css' );
}
...
if ( 2 !== (int) $columns && 'grid' == $view ) {
	$output = str_replace( '<ul class="product_list_widget has-ccols ccols-2" role="none">', '<ul class="product_list_widget has-ccols ' . esc_attr( function_exists( 'porto_generate_column_classes' ) ? porto_generate_column_classes( $columns ) : '' ) . '" role="none">', $output );
	...
	$output  = str_replace( '<ul class="product_list_widget has-ccols ccols-2" role="none">', '<ul class="' . esc_attr( $wrapper_class ) . '" data-plugin-options="' . esc_attr( $options ) . '" role="none">', $output );
}

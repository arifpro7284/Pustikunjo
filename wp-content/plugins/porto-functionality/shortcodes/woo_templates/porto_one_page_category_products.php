...
$subcategory_class = 'sub-category products has-ccols ccols-xl-' . $columns . ' ccols-lg-' . $cols_md . ' ccols-md-' . $cols_md . ' ccols-sm-' . $cols_xs . ' ccols-' . $cols_ls;
...
$output .= '<ul class="product-cats"' . ( ( ! empty( $show_products ) && empty( $sticky_pos ) ) ? ' data-plugin-sticky' : '' ) . ' data-plugin-options="' . esc_attr( '{"autoInit": true, "minWidth": 768, "containerSelector": "' . ( $show_products ? '.porto-onepage-category' : '#main' ) . '","autoFit":true, "paddingOffsetTop": 1}' ) . '">';
	...
	foreach ( $terms as $term_cat ) {
		...
		$output                  .= '<div class="category-title">';
			$output              .= '<div class="dropdown">';
				...
				// 	)
				// );
				// $output          .= '<h4 class="cat-title dropdown-toggle' . ( $child_categories ? ' has-sub-cat' : '' ) . '" data-display="static" ' . ( $child_categories ? 'data-bs-toggle="dropdown" aria-expanded="false"' : '' ) . '><span>' . esc_html( $term_cat->name ) . '</span></h4>';
				$output          .= '<h4 class="cat-title" data-display="static"><span>' . esc_html( $term_cat->name ) . '</span></h4>';
		// if ( $child_categories ) {

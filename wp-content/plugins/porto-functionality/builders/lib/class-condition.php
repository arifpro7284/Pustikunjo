	...
	class Porto_Builder_Condition {
		...
		public function ajax_search( $direct_call = false ) {
			...
			if ( ! empty( $_REQUEST['post_type'] ) ) {
				$search_type = sanitize_text_field( $_REQUEST['post_type'] );
				...
				if ( 'post' == $type && post_type_exists( $search_type ) ) {
					$type       = 'post';
					$type_query = ' AND post_type="' . $search_type . '"';
		...
		public function save_condition( $direct_call = false, $post_id = false ) {
			...
			/**
			 * Save the conditions AFTER wp_update_post().
			 *
			 * wp_update_post() fires save_post, and with WPML active that runs WPML's custom-field
			 * synchronization, which reverts the just-written ( unconfigured, serialized )
			 * _porto_builder_conditions meta back to its pre-save value. Writing it here — after the
			 * post update, so no further save_post fires for this post — makes it persist.
			 * ( See also wpml-config.xml for declaring these plugin meta keys. )
			 *
			 * @since 3.9.0
			 */

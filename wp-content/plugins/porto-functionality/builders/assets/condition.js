...
jQuery( window ).on( 'load', function () {
	setTimeout( function () {
		...
		if ( jQuery( '#preview_width' ).length ) {
			...
			let editorRoot = document.querySelector('.editor-styles-wrapper');
			if ( ! editorRoot ) {
				let iframeObj = document.querySelector('.editor-visual-editor iframe[name=editor-canvas]');
				if ( iframeObj ) {
					const iframeDoc = iframeObj.contentDocument || iframeObj.contentWindow.document;
					editorRoot = iframeDoc.querySelector('.editor-styles-wrapper');
				}
			}

			$preview_width_obj.on( 'keyup', function ( e ) {
				...
				preview_width_trigger = setTimeout( function () {
					if ( !editorRoot || !document.contains(editorRoot) ) {
						editorRoot = document.querySelector('.editor-styles-wrapper');
						if ( ! editorRoot ) {
							let iframeObj1 = document.querySelector('.editor-visual-editor iframe[name=editor-canvas]');
							if ( iframeObj1 ) {
								const iframeDoc1 = iframeObj1.contentDocument || iframeObj1.contentWindow.document;
								editorRoot = iframeDoc1.querySelector('.editor-styles-wrapper');
							}
						}
					}
					if (editorRoot) {
						editorRoot.style.setProperty('--porto-g-editor-w', val + 'px');
					}
				}, 300 );
			} );
		}

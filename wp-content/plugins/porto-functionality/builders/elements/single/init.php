...
if ( ! class_exists( 'PortoBuildersSingle' ) ) :
	class PortoBuildersSingle {
		...
		public function init() {
			...
			add_action( 'init', array( $this, 'find_preview' ) ); // For Elementor Editor
		}
		...
		public function elementor_custom_single_shortcodes( $self ) {
			...
			if ( apply_filters( 'porto_elementor_load_builder_widgets', $load_widgets, 'single' ) ) {
				...
				// Post Content element is Elementor-only ( intentionally kept out of $this->shortcodes
				// so no WPBakery shortcode / element is registered for it ).
				include_once PORTO_BUILDERS_PATH . 'elements/single/elementor/post-content.php';
				if ( class_exists( 'Porto_Elementor_Single_Post_Content_Widget' ) ) {
					$self->register( new Porto_Elementor_Single_Post_Content_Widget( array(), array( 'widget_name' => 'Porto_Elementor_Single_Post_Content_Widget' ) ) );
				}
			}
		}
		...
		public function register_elementor_preview_controls( $document ) {
			...
			if ( $id && 'single' == get_post_meta( get_the_ID(), PortoBuilders::BUILDER_TAXONOMY_SLUG, true ) ) {
				if ( empty( $this->edit_post_type ) ) {
					$edit_post_type = get_post_meta( $id, 'preview_type', true );
					if ( ! empty( $edit_post_type ) ) {
				...
				foreach ( $_post_types as $post_type => $object ) {
					if ( ! in_array( $post_type, array( 'page', 'product', 'e-landing-page', 'wpb_gutenberg_param' ) ) ) {
				...
				$document->start_controls_section(
					...
					$document->add_control(
						...
						array(
							...
							'default'     => ! empty( $this->edit_post_type ) ? $this->edit_post_type : 'post',
							'groups'      => array(
								'single' => array(
		...
		public function apply_preview_post() {
			...
			if ( empty( $_REQUEST['post_id'] ) || empty( $_REQUEST['mode'] ) ) {
			...
			update_post_meta( $post_id, 'preview_type', sanitize_title( $_REQUEST['mode'] ) );
			die;
		}

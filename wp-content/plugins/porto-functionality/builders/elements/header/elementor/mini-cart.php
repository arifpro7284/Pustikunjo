...
class Porto_Elementor_HB_Mini_Cart_Widget extends \Elementor\Widget_Base {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'label'       => __( 'Open Mini Cart After Add to Cart', 'porto-functionality' ),
					'description' => __( 'Check this option to open the mini cart after a product is added to the cart on the single product page. This works only when using add to cart by AJAX.', 'porto-functionality' ),
					'condition'   => array(
						'content_type' => array( 'offcanvas' )
					)
				)
			);
	...
	protected function render() {
		...
		if ( function_exists( 'porto_header_elements' ) ) {
			...
			porto_header_elements( array( (object) array( 'mini-cart' => '' ) ), class_exists( 'WooCommerce' ) && is_product() && ! empty( $settings['open_minicart'] ) ? 'open-minicart-added' : '' );

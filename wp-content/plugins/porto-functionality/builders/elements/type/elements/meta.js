...
( function ( wpI18n, wpBlocks, wpBlockEditor, wpComponents ) {
    ...
    const PortoTBMeta = function ( { attributes, setAttributes, name, clientId } ) {
        ...
        useEffect(
            () => {
                if ( ! attributes.el_class || -1 !== porto_tb_ids.indexOf( attributes.el_class ) ) { // new or just cloned
                    let new_cls = 'porto-tb-meta-' + Math.ceil( Math.random() * 10000 );
        ...
        let attrs = Object.assign( {}, { field: attributes.field, date_format: attributes.date_format, icon_cls: attributes.icon_cls, icon_pos: attributes.icon_pos, el_class: attributes.el_class, className: attributes.className, font_settings: attributes.font_settings, style_options: style_options, ...style_options } );
    ...
    registerBlockType( 'porto-tb/porto-meta', {
        ...
        icon: 'porto',
        category: 'porto-tb',
        keywords: [ 'type builder', 'mini', 'brands', 'card', 'post date', 'post published date', 'post modified date', 'author', 'categories', 'comments count', 'product sku', 'brand name', 'GTIN', 'UPC', 'EAN', 'ISBN' ],

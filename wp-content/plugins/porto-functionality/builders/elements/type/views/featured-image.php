...
if ( ( $current_object = get_queried_object() ) && ! empty( $current_object->term_id ) ) {
	...
	} elseif ( function_exists( 'porto_get_featured_images' ) ) {
...
if ( isset( $atts['hover_effect'] ) ) {
	...
	if ( ! empty( $overlay_image ) ) {
		wp_enqueue_script( 'porto-pg-hv-overlay' );
...
if ( ! empty( $image_type ) && ( 'hover' == $image_type || 'slider' == $image_type || 'gallery' == $image_type ) ) {
	...
	if ( empty( $attachment_ids ) && function_exists( 'porto_get_featured_images' ) ) {
...
if ( count( $attachment_ids ) > 1 && ( 'slider' == $image_type || 'gallery' == $image_type ) ) {
	...
	foreach ( $attachment_ids as $img_id ) {
		...
		if ( ! $attachment ) {
			$attachment = array(
				'src'     => ! empty( $product ) ? wc_placeholder_img_src( 'woocommerce_single' ) : PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg',
				'caption' => '',
				'alt'     => '',
			);
			// continue;
		...
		echo '<a aria-label="post featured image gallery" href="' . esc_url_raw( $image_link ) . '"' . ( $link_target ? ' target="' . esc_attr( $link_target ) . '"' : '' ) . ' class="img-thumbnail">';

		$res_html = wp_get_attachment_image( is_array( $img_id ) ? $img_id['attachment_id'] : $img_id, $image_size, false, array( 'class' => 'img-responsive' ) );
		if ( empty( $res_html ) ) {
			if ( ! empty( $product ) ) {
				$place_link = wc_placeholder_img_src( 'woocommerce_single' );
			} else {
				$place_link = PORTO_SHORTCODES_URL . 'assets/images/placeholder.jpg';
			}
			echo '<img class="img-responsive" src="' . $place_link . '" />';
		} else {
			echo porto_filter_output( $res_html );
		}
		...
		$large_src[] = array( $attachment['src'], $attachment['caption'], $attachment['alt'] );
	...
	if ( 'slider' == $image_type ) {
		...
		if ( ! empty( $atts['zoom'] ) ) {
			...
			foreach ( $large_src as $full_src ) {
				echo '<a title="' . esc_attr( $full_src[2] ) . '" data-title="' . esc_attr( $full_src[1] ) . '" href="' . esc_url( $full_src[0] ) . '"></a>';
			}
			echo '</div>';
		}
	}
	...
	if ( ! empty( $video_html ) ) {
		...
		if ( -1 == $image_id ) {
			...
			if ( false === $image_id && 'video' == $image_type ) {
			} else {
				$res_html = wp_get_attachment_image( $image_id, $image_size, false, array( 'class' => 'img-responsive' ) );
				if ( empty( $res_html ) ) {
					if ( ! empty( $product ) ) {
						$place_link = wc_placeholder_img_src( 'woocommerce_single' );
					} else {
...
if ( ! empty( $atts['zoom'] ) && ( ( 'slider' != $image_type && 'gallery' != $image_type ) || ! empty( $is_gallery ) ) && empty( $video_html ) ) {

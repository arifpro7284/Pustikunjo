...
if ( ! class_exists( 'PortoBuildersType' ) ) :
	class PortoBuildersType {
		...
		public function init() {
			...
			if ( $screen && 'post' == $screen->base && PortoBuilders::BUILDER_SLUG == $screen->id ) {
				...
				if ( $screen->is_block_editor() ) {
					...
					add_action( 'enqueue_block_assets', array( $this, 'enqueue' ), 1000 );
					add_filter(
		...
		public function save_meta_values( $post_id, $post ) {
			if ( ! $post || ! isset( $post->post_type ) || PortoBuilders::BUILDER_SLUG != $post->post_type || 'type' != get_post_meta( $post_id, PortoBuilders::BUILDER_TAXONOMY_SLUG, true ) ) {
				return;
			}
		...
		private function get_meta_box_fields() {
			...
			$disabled_post_types = array( 'attachment', 'porto_builder', 'page', 'e-landing-page', 'wpb_gutenberg_param' );
		...
		public function enqueue() {
			...
			if ( doing_action( 'porto_enqueue_css' ) ) {
				return;
			}
			...
			// Editor chrome + preview styling, enqueued via enqueue_block_assets ( the supported
			// path for iframe styles ) instead of inline CSS on the theme's porto-blocks-editor
			// handle, which Gutenberg flagged as added to the iframe incorrectly.
			wp_add_inline_style( 'porto-type-builder', 'body.post-type-porto_builder #elementor-editor, body.post-type-porto_builder .elementor-switch-mode-on { display: none !important; } .post-type-porto_builder #elementor-switch-mode, .post-type-porto_builder .composer-switch { display: none } .edit-post-visual-editor, .edit-post-visual-editor__content-area > div { background: #333 !important } .editor-styles-wrapper{min-width: auto;--porto-g-editor-w:' . floatval( $preview_width ) . 'px;height: auto !important;margin: 30px auto;padding:0 20px 4rem}.editor-styles-wrapper .wp-block{margin-left: 0; margin-right: 0;}' );
		}
				...
				array(
					...
					'params'      => array_merge(
						array(
							...
							array(
								'type'       => 'porto_param_heading',
								'param_name' => 'recent_tip',
								'text'       => sprintf( esc_html__( '%1$sIf you select Recent Posts, please set "OrderBy" to "Date" and "Order" to "Descending."%2$s', 'porto-functionality' ), '<b style="color: #08c">', '</b>' ),
							),
							array(
								'heading'    => __( 'Product Visibility', 'porto-functionality' ),
								'type'       => 'dropdown',
								'param_name' => 'product_visibility',
								'default'    => '',
								'value'      => array(
									__( 'Shop and search results', 'porto-functionality' ) => 'visible',
									__( 'Shop only', 'porto-functionality' )               => 'catalog',
									__( 'Search results only', 'porto-functionality' )     => 'search',
									__( 'Hidden', 'porto-functionality' )                  => 'hidden',
									__( 'Any', 'porto-functionality' )                     => 'any',
								),
								'dependency' => array(
									'element' => 'post_type',
									'value'   => 'product',
							...
							array(
								...
								'selectors'  => array(
									'{{WRAPPER}} .sort-source a' => 'padding-top: {{TOP}};padding-right: {{RIGHT}};padding-bottom: {{BOTTOM}};padding-left: {{LEFT}};',
								),
								'group'       => __( 'Style', 'porto-functionality' ),
							),
							array(
								...
								'param_name' => 'filter_it_radius',
								'dependency'  => array(
									'element'   => 'category_filter',
									'not_empty' => true,
								),
								'selectors'  => array(
									'{{WRAPPER}} .sort-source a' => 'border-top-left-radius: {{TOP}};border-top-right-radius: {{RIGHT}};border-bottom-right-radius: {{BOTTOM}};border-bottom-left-radius: {{LEFT}};',
								),
								'group'       => __( 'Style', 'porto-functionality' ),
							),
							array(
			...
			vc_map(
				array(
					...
					'params'      => array(
						...
						array(
							'type'        => 'checkbox',
							'heading'     => __( 'Preserve the page number in the URL', 'porto-functionality' ),
							'description' => __( 'Enable this option to update the page number in the URL when new products are loaded.', 'porto-functionality' ),
							'param_name'  => 'load_more_keep_url',
							'std'         => '',
							'dependency'  => array(
								'element' => 'pagination_style',
								'value'   => array( 'infinite', 'load_more' ),
							),
						),
						array(
							'type'        => 'checkbox',
		...
		protected function render_block( $atts, $block_name, $content = null ) {
			...
			if ( ( ( isset( $_GET['action'] ) && 'elementor' != $_GET['action'] && is_admin() && ! empty( get_current_screen() ) && get_current_screen()->is_block_editor() ) || wp_is_json_request() || defined( 'REST_REQUEST' ) || ( isset( $_REQUEST['action'] ) && false !== strpos( $_REQUEST['action'], 'porto_import' ) ) ) && ( empty( $_POST['action'] ) || 'elementor_ajax' != $_POST['action'] ) && ! apply_filters( 'porto_is_tb_rendering', false ) ) { // in block editor ( but not while looping real posts inside a Posts Grid / archive, where the loop already set up $post / $product )
		...
		public function filter_search_loop( $query ) {
			if ( ! is_admin() && $query->is_main_query() && ( $query->is_home() || $query->is_search() || $query->is_archive() ) ) {
				...
				if ( ! $post_type ) {
					$post_types_exclude   = apply_filters( 'porto_condition_exclude_post_types', array( PortoBuilders::BUILDER_SLUG, 'attachment', 'elementor_library', 'page', 'ae_global_templates', 'wpb_gutenberg_param' ) );

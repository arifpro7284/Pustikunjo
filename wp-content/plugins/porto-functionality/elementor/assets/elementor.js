	...
	if ( typeof elementorFrontend != 'undefined' ) {
		var porto_elementor_init = function() {
			...
			( function quick_access() {
				$( document.body ).on( 'mouseenter', '.elementor-element-editable.elementor-element', function() {
					...
					if ( portoQuickAccessCache[widgetKey] ) {
						...
						if ( widgetKey == 'porto_sidebar_menu' || widgetKey == 'porto_hb_menu' || widgetKey == 'porto_hb_toggle' ) {
							if ( nonPosElements.length ) {
								$this.on( 'mouseenter', '.elementor-widget-container>ul>li.has-sub, .mega-menu>li.has-sub, .sidebar-menu>li.has-sub, .sidebar-menu .narrow .inner>.sub-menu>li.menu-item-has-children', function( e ) {
								...
								} ).on( 'mouseleave', '.elementor-widget-container>ul>li.has-sub, .mega-menu>li.has-sub, .sidebar-menu>li.has-sub', function( e ) {
									var $related = $( e.relatedTarget );
									if ( $related.closest( '.porto-qa-item.non-pos' ).length ) {
			...
			if ( typeof porto_init == 'function' ) {
				...
				elementorFrontend.hooks.addAction( 'frontend/element_ready/container', function( $obj ) {
					...
					if ( $obj.children( '.content-marker' ).length ) {
						$obj.addClass( 'gsap-content-marker' ).attr( 'data-marker-content', $obj.children( '.content-marker' ).attr( 'marker-content' ) );
					} else {
						$obj.removeClass( 'gsap-content-marker' ).removeAttr( 'data-marker-content' );
					}
					...
					if ( typeof $obj.children( '.porto-cl-floating' ).attr( 'data-plugin-float-element' ) != 'undefined' ) {
						var opts = $obj.children( '.porto-cl-floating' ).data( 'plugin-options' );
						if ( typeof opts == 'string' ) {
							try {
								opts = JSON.parse( opts.replace( /'/g, '"' ).replace( ';', '' ) );
							} catch ( e ) { }
						}
						$obj.removeData( '__floatElement' );
						$obj.themePluginFloatElement( opts );
					} else if ( typeof $obj.data( '__floatElement' ) != 'undefined' ) {
						$obj.trigger( 'float_destroy' );
					}

					$obj.removeClass( 'porto_stretch_css stretch_row_css stretch_row_content_css' );
					if ( $obj.children( '.porto_stretch_css' ).length ) {
						var $wrapper = $obj.children( '.porto_stretch_css' );
						$obj.addClass( $wrapper.attr( 'class' ).replace( /d-none/gi, '' ) );
					}
				} );
			...
			function initWidgetAddon( $obj ) {
				...
				if ( 'object' == typeof widget_settings && widget_settings.mpx ) {
					...
					if ( $obj.hasClass( 'elementor-element' ) ) {
						if ( $obj.attr( 'data-widget_type' ) && $obj.children( '.elementor-widget-container' ).length == 0 ) {
							$obj.wrapInner( '<div class="layer"></div>' );
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_sticky_nav.default', function( $obj ) {
				$obj.find( '.porto-sticky-nav' ).each( function() {
					var $this = $( this ),
						opts;
					...
					if ( pluginOptions )
						opts = pluginOptions;
					if ( $this.is( ':visible' ) && $.fn.themeSticky ) {
			...
			elementorFrontend.hooks.addAction( 'refresh_popup_options', function( option, value ) {
				if ( 'popup_width' == option ) {
				...
				} else if ( 'offcanvas' == option ) {
					$( document.body ).addClass( 'porto-popup-offcanvas' );
				} else {
					var _$ = value;
					...
					if ( option == 'popup_pos_first' ) {
						...
						if ( 'offcanvas' == elementor.settings.page.model.get( 'popup_type' ) ) {
							$( document.body ).addClass( 'porto-popup-offcanvas' );
						}
						setTimeout(() => {
							$( '.elementor.elementor-edit-area' ).css( 'max-width', elementor.settings.page.model.get( 'popup_width' ) + 'px' );
						}, 1200);
						
					} else {
						if ( ! _$( 'select[data-setting="popup_type"]' ).val() ) {
							$( document.body ).removeClass( 'porto-popup-offcanvas' );
						}
					}
			...
			if ( typeof elementorFrontend.elementsHandler.elementsHandlers.section[4] == 'function' && elementorFrontend.elementsHandler.elementsHandlers.section[4].prototype && elementorFrontend.elementsHandler.elementsHandlers.section[4].prototype.buildSVG ) {
			...
			} else if ( typeof elementorFrontend.elementsHandler.elementsHandlers.section[4] == 'function' ) {
				elementorFrontend.elementsHandler.elementsHandlers.section[4]().then( result => {
		...
		if ( typeof elementorPro != 'object' ) {
			elementorFrontend.on( 'components:init', function() {
				...
				createHandles();
				window.elementor && elementor.on( 'document:loaded', function() {
					createHandles();
				} );
			} );
		}

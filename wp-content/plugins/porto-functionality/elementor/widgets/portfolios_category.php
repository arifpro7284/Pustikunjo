...
class Porto_Elementor_Portfolios_Category_Widget extends \Elementor\Widget_Base {
	...
	public function get_script_depends() {
		if ( isset( $_REQUEST['elementor-preview'] ) ) {
			return array( 'isotope', 'porto-elementor-widgets-js', 'porto-bg-parallax' );
		} else {
			return array();
		}
	}
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'.elementor-element-{{ID}} .thumb-info-bottom-info:not(.thumb-info-bottom-info-dark) .thumb-info-inner' => 'color: {{VALUE}};',
				),
			)
		);
		$this->add_control(

...
class Porto_Elementor_Ultimate_Heading_Widget extends \Elementor\Widget_Base {
	...
	public function get_keywords() {
		return array( 'title', 'text', 'marquee', 'highlight', 'floating image', 'typewriter', 'post content', 'single builder post title' );
	}
	...
	public function get_script_depends() {
		...
		if ( isset( $_REQUEST['elementor-preview'] ) ) {
			...
			$depends[] = 'porto-animated-letters';
			$depends[] = 'porto-hover-image';
		}
		return $depends;
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'description' => sprintf( __( 'e.g: fadeInUp, fadeIn and so on. Please check this %1$surl%2$s to see the animation name.', 'porto-functionality' ), '<a href="https://www.portotheme.com/wordpress/porto/shortcodes/animations/" target="_blank">', '</a>' ),
				'default'     => 'fadeIn',
				'condition'   => array(
					'enable_typewriter' => 'yes',
				),
		...
		$this->add_control(
			...
			array(
				...
				'options'   => array(
					...
					'underline2'       => __( 'Underline 2', 'porto-functionality' ),
				),
				'default'   => 'scribble',
				'condition' => array(
					'enable_highlight'    => 'yes',
	...
	protected function content_template() {
		...
		<#
			...
			if ( settings.main_heading ) {
				...
				if ( settings.enable_highlight ) {
					if ( settings.enable_svghighlight ) {
						...
						} else if ( 'underline2' == settings.highlight_svg ) {
						...
						view.addRenderAttribute( 'main_heading_wrap', 'data-appear-animation', 'highlightShape' );
					} else {
			...
			if ( settings.enable_typewriter ) {
				...
				if( settings.enable_type_loop ) {
					typewriter[ 'loop' ] = settings.enable_type_loop;
				}

	...
	class Porto_Elementor_Init {
				...
				function( $document ) {
					...
					if ( 'porto_builder' == $document->get_post()->post_type && $document->get_post()->ID && 'popup' == get_post_meta( $document->get_post()->ID, 'porto_builder_type', true ) ) {
						...
						$document->add_control(
							...
							array(
								'type'    => Elementor\Controls_Manager::SELECT,
								'label'   => esc_html__( 'Popup Type', 'porto-functionality' ),
								'options' => array(
									''          => __( 'Modal', 'porto-functionality' ),
						...
						$document->add_control(
							...
							array(
								'type'    => Elementor\Controls_Manager::SELECT,
								'label'   => esc_html__( 'Position', 'porto-functionality' ),
								'options' => array(
									''      => __( 'Left (Right on RTL)', 'porto-functionality' ),
									'right' => __( 'Right (Left on RTL)', 'porto-functionality' ),
								),
								'condition' => array(
									'popup_type' => 'offcanvas',
								),
							)
						);
						...
						$document->add_control(
							...
							array(
								...
								'selectors' => array(
									'.mfp-content [data-popup-id="' . $document->get_post()->ID . '"], .elementor-' . $document->get_post()->ID => 'background-color: {{VALUE}} !important;',
								),
							)
						);
						...
						$document->add_control(
							...
							array(
								'type'        => Elementor\Controls_Manager::SWITCHER,
								'label'       => __( 'Disable Overlay', 'porto-functionality' ),
								'description' => __( 'Disable the popup overlay.', 'porto-functionality' ),
							)
						);
				...
				add_action(
					...
					function( $self, $data ) {
						...
						if ( isset( $post_id ) && 'popup' == get_post_meta( $post_id, 'porto_builder_type', true ) ) {
							$popup_options                  = array();
							$popup_options['width']         = wp_slash( isset( $data['settings']['popup_width'] ) && '' != $data['settings']['popup_width'] ? $data['settings']['popup_width'] : 740 );
							$popup_options['animation']     = ! empty( $data['settings']['popup_animation'] ) ? wp_slash( $data['settings']['popup_animation'] ) : 'mfp-fade';
							$popup_options['load_duration'] = wp_slash( isset( $data['settings']['load_duration'] ) && '' != $data['settings']['load_duration'] ? $data['settings']['load_duration'] : 4000 );
							...
							if ( ! empty( $data['settings']['popup_type'] ) ) {
								$popup_options['popup_type'] = $data['settings']['popup_type'];
							}
							if ( ! empty( $data['settings']['popup_offcanvas_pos'] ) ) {
								$popup_options['popup_offcanvas_pos'] = $data['settings']['popup_offcanvas_pos'];
							}
							if ( ! empty( $data['settings']['enable_exit_intent'] ) ) {
								$popup_options['enable_exit_intent'] = $data['settings']['enable_exit_intent'];
							}
							if ( ! empty( $data['settings']['disable_overlay'] ) ) {
								$popup_options['disable_overlay'] = $data['settings']['disable_overlay'];
							}
		...
		public function register_widget_styles() {
			...
			foreach ( $animations as $animation ) {
				$enqueue_styles[ 'e-animation-' . $animation ] = array(
					'src' => PORTO_CSS . '/part/' . $animation . '.css',
				);
			}

			foreach ( $enqueue_styles as $handle => $style ) {
				wp_deregister_style( $handle );
				wp_register_style( $handle, $style['src'], ! empty( $style['deps'] ) ? $style['deps'] : array(), isset( $style['ver'] ) ? $style['ver'] : PORTO_VERSION );
			}
		}
		...
		public function after_page_first_print( $document, $is_excerpt, $with_css ) {
			...
			$is_saved = get_post_meta( $post_id, 'porto_elementor_page_assets_saved', true );
		...
		/**
		 * Update assets after Elementor block saves
		 * 
		 * @since 3.5.0
		 */
		public function after_elementor_editor_save( $post_id, $editor_data ) {
			if ( ! $this->is_elementor_cache_enabled() ) {
				...
				delete_post_meta( $post_id, 'porto_elementor_page_assets_saved' );
			}
		}

		/**
		 * Create the page assets for Elementor blocks
		 * 
		 * @since 3.5.0
		 */
		public function create_elementor_page_assets( $post_id, $editor_data, $with_css = false ) {
			...
			// Cache compatiblity of Porto Builder
			if ( 'porto_builder' == get_post_type( $post_id ) && $this->is_elementor_cache_enabled() ) {
			...
			update_post_meta( $post_id, 'porto_elementor_page_assets_saved', true );
		}
		...
		public function register_elementor_widgets( $self ) {
			...
			if ( Plugin::$instance->experiments->is_feature_active( 'e_optimized_markup' ) ) {
				$self->unregister( 'common-optimized' );
				include_once dirname( PORTO_META_BOXES_PATH ) . '/elementor/widgets/common-optimized.php';
				$self->register( new Porto_Elementor_Common_Optimized_Widget( array(), array( 'widget_name' => 'common-optimized' ) ) );
			}
		...
		public function load_elementor_widgets_js() {
			if ( ( isset( $_REQUEST['action'] ) && 'elementor' == $_REQUEST['action'] ) || isset( $_REQUEST['elementor-preview'] ) ) {
				...
				wp_enqueue_script( 'porto-hz-scroller' );
				...
				wp_enqueue_script( 'porto-float-element' );
				if ( defined( 'PORTO_CSS' ) ) {
					wp_enqueue_style( 'porto-content-collapse', PORTO_CSS . '/part/content-collapse.css', array( 'porto-theme' ), PORTO_VERSION );
					wp_enqueue_style( 'porto-hv-split', PORTO_CSS . '/part/hv-split.css', array( 'porto-theme' ), PORTO_VERSION );
					wp_enqueue_style( 'porto-hz-scroller', PORTO_CSS . '/part/hz-scroller.css', array( 'porto-theme' ), PORTO_VERSION );
					wp_enqueue_style( 'porto-inviewport', PORTO_CSS . '/part/view-scale.css', array( 'porto-theme' ), PORTO_VERSION );
				}
		...
		public function elementor_add_animation( $additional_animations ) {
			...
			$additional_animations['Text'] = array( 'textEntrance' => 'Text Entrace' );

...
class Porto_Elementor_Section extends Elementor\Element_Section {
	...
	public function after_render() {
		...
		if ( 'banner' == $settings['as_param'] && ! empty( $settings['banner_link']['url'] ) ) {
			echo '<a aria-label="Go to banner link" class="porto-ibanner-link" href="' . esc_url( $settings['banner_link']['url'] ) . '"' . ( $settings['banner_link']['is_external'] ? ' target="_blank"' : '' ) . '></a>';
		} elseif ( 'creative' == $settings['as_param'] ) {
			$grid_sizer = '';
	...
	protected function section_tab_nav( $nav_tab = '' ) {
		...
		?>
		<ul role="tablist" class="nav nav-tabs<?php echo esc_attr( $nav_tab ); ?>">
			<?php foreach ( $porto_section['tab_data'] as $idx => $data ) : ?>
				...
				echo ( 0 === $idx ? ' active' : '' );
				?>" pane-id="<?php echo esc_attr( $data['id'] ); ?>" ><a role="tab" class="nav-link<?php echo ( 0 === $idx ? ' active' : '' ); ?>" href="#" data-tab="tab-<?php echo esc_attr( $data['id'] ); ?>"><?php echo porto_strip_script_tags( $html ); ?></a></li>
			<?php endforeach; ?>
		</ul>
		<?php
	}
...
function porto_elementor_shape_divider( $self, $args ) {
	...
	$shapes_options = Elementor\Shapes::get_shapes();
	$shapes_options['custom'] = array(
		'title' => esc_html__( 'Custom', 'porto-functionality' ),
		'image' => PORTO_SHORTCODES_URL . 'assets/images/custom.svg',
	);
...
function porto_elementor_print_section_template( $content, $self ) {
	...
	$content .= '<div class="porto-el-section-shortcodes"><a href="#" class="porto-el-section-shortcode porto-additional-option" title="Section Additional Settings"><i class="porto-icon-cog"></i></a>';
	if ( apply_filters( 'porto_header_builder', false ) ) {
		// Shortcode for Sticky Section Header 
		$content .= '<a href="#" class="porto-el-section-shortcode porto-header-tooltip" title="Section Sticky"><i class="porto-icon-pencil"></i></a>';
	}
	$content .= '</div>';
...
function porto_elementor_section_add_custom_attrs( $self ) {
	...
	if ( 'h_scroller' == $settings['as_param'] || ( isset( $settings['scroll_bg_scale'] ) && 'yes' == $settings['scroll_bg_scale'] ) ) {
		...
		if ( defined( 'PORTO_CSS' ) ) {
			porto_enqueue_link_style( 'porto-hz-scroller', PORTO_CSS . '/part/hz-scroller.css' );
		}
	}
